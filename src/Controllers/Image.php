...
class Image extends ImageModel
{
  ...
  public static function SaveUploadImage($type, $base64data, $user_id = 'cache')
  {
    ...
    try {
      foreach ($sizeArray as $key => $size) {
        ...
        //保存记录到数据库
        // $val = self::AddImageRecord($type, null, $user_id, $locale_img, imagesx($image), imagesy($image));
    ...
    // 删除临时文件
    unlink($locale_img);//暂时不删除。用于保存用户上传的原图
   ...
   * @return bool 是否添加成功
   */
  public static function AddImageRecord($type, $item_id, $user_id, $url,$width,$height)
  {
    // $image = new self();
    // $image->key = md5($url);
    // $image->filename = $url;
