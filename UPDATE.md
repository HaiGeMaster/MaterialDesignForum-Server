...
## Material Design Forum UPDATE.MD

### 版本更新日志

##### 未提供更新日志时，则为仅更新了一些微不足道的地方，例如ui布局变动，ui调整功能等，无关数据交互类。
...
**新增内容**
 - 轮播图、话题卡片、用户卡片、部分展示列表更新为纯vuetify组件。
 - 文章页、提问页、回答组件、评论组件、回复组件更新为纯vuetify组件。
...
**修复**
 - 您需要手动从新版本的`.env.example`模板文件中复制`OAUTH2_GOOGLE_CLIENT_ID`和`OAUTH2_GOOGLE_CLIENT_SECRET`的键值到您的`.env`配置文件中，否则会导致OA登录系统异常。
...
#### 2026-02-26 版本更新
**核心变更：引入 `.env` 配置文件**
为提升安全性与部署便捷性，本次更新引入了环境变量配置文件。以下为关键变更与操作指南：

**一、 新增配置文件**
- 新增 `.env` 环境变量配置文件，用于集中管理敏感信息与系统配置。
- 部署包中默认包含模板文件 `.env.example`，您需要基于此模板创建实际的 `.env` 文件。

**二、 配置与部署说明**
1.  **首次安装**：请根据网站根目录下的 `.env.example` 模板，创建并配置您的 `.env` 文件。此举为安装程序正常运行的必要步骤。
2.  **数据库初始化**：首次安装时，您仍需手动将 `assets/demo_table.sql` 文件导入数据库。
3.  **后续更新**：完成首次配置后，后续版本更新将无需在 `Config.php` 中重复配置数据库连接信息，系统会自动读取 `.env` 中的配置。

**三、 安全注意事项**
- **存储内容**：请在 `.env` 文件中配置所有敏感信息，如数据库密码、邮件服务凭证等。系统将不再于数据库中存储此类信息。
- **文件权限**：为确保安全，**务必**将 `.env` 文件权限设置为 `640` 或更严格的权限级别，以防止未授权访问与信息泄露。
- **路径**：`.env` 该文件位于网站根目录。
- **管理**：部分在 `.env` 中配置的信息，仍可通过系统后台设置界面进行修改。

**四、 故障处理**
若系统无法自动创建 `.env` 文件，请手动创建并参照 `.env.example` 的格式完成配置。
...
#### 2026-01-23 版本更新
**新增功能**
 - `登录` `注册` `找回密码`时的提示将以`Snackbar`提示展现
 - 添加了通知全部删除功能
 - 继续最低限度的兼容PHP7.4

**修复**
 - 修复了`vuetify2`主题重置密码时的参数验证失败问题
...
#### 2025-11-24 版本更新
**修复**
 - 修复注册用户在注册时出现两个以上IP无法取地址值的情况。
 - 修复提问下的回答列表打开对应评论对话框时，评论会错误拉取全部评论的问题。
...
#### 2025-10-30 版本更新
**新增功能**
 - 新增SSO登录功能，前端双主题（vuetify2，mdui2）、后端均已适配。
 - 优化用户变更用户组时，用户组数量未正确同步更新问题

---

#### 2025-10-9 版本更新
**修复**
 - 修复Vuetify2主题客户端通知数据类型为user_follow展示不全问题。

---

#### 2025-10-7 版本更新
**修复**
 - 修复MDUI2主题客户端通知数据展示不全问题。
